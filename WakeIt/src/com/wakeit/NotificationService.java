...
public class NotificationService extends Service implements SensorEventListener{
  ...
  final public static int timeToShake=100;// how much time until the subject is fully awake. in sec.
  //final public static int checkInterval=5;// how much time of not-moving means being asleep. in sec.
  ...
  private final class VolumeIncreaser implements Runnable {
    ...
    public void run() {
    	...
    	int fci=0;//final check interval
		switch(this.idx){
		case 0: fci=4; break;
		case 1: fci=8; break;
		case 2: fci=8; break;
    	...
    	if(clockFlag){//we are belived to be asleep
    		//MediaSingleton.INSTANCE.vibrate();
    		counter=0;
    		if(this.idx==0){
    			jdx++;
	      ...
	      if(speed >= SHAKE_THRESHOLD){
	    	  ...
	    	  clockFlag = false;
	    	  //MediaSingleton.INSTANCE.cancelVibrate();
	      }
	      ...
	      MediaSingleton.INSTANCE.setVolume(start);//update sound
    		...
    		counter++;//adds sec
    		Toast.makeText(getBaseContext(),"counter = "+counter, Toast.LENGTH_SHORT).show();
    	...
    	if (counterTotal <= timeToShake) {
    		handler.postDelayed(volumeIncreaseCallback, 500);
    	}
    	else {
