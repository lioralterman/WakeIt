...
public final class AppSettings {
  ...
  public static final String TER_BTN_MODE = "TERMINATOR_BUTTON";
  ...
  public static final boolean isDebugMode(Context c) {
    ...
    final String DEBUG_ON = values[0];
    final String DEBUG_OFF = values[1];
  ...
  public static final boolean isTerButtonOn(Context c){
	  	final String[] values = c.getResources().getStringArray(R.array.ter_btn_values);
	  	final String BTN_ON = values[0];
    	final String BTN_OFF = values[1];
    	
	    final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(c);
	    final String value = prefs.getString(TER_BTN_MODE, BTN_OFF);
	    if (value.equals(BTN_ON)) {
	      return true;
	    } else if (value.equals(BTN_OFF)) {
	      return false;
	    } else {
	      throw new IllegalStateException("Unknown btn mode mode setting: "+ value);
	    }
  }
  public static final int alarmTimeOutMins(Context c) {
    return 120; /* SHOULD CHANGE, UGLY CODING
